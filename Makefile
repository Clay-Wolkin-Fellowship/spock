...
L1PLOTS = $(foreach rep,$(REPS),$(TRACES:%=build/plotmc1/%.$(rep).png))
...
$(SAMPLES): $$(ZIP) build/bin/sample
	@mkdir -p $(@D)
	build/bin/sample $< $(SAMPN) $(SAMPK) >$@

$(L1CACHE): $$(SAMPLE) rep/$$(REPL).py
	@mkdir -p $(@D)
	build/bin/uncompress <$< | python3 rep/$(REPL).py $(L1ARGS) - $@

$(L1MATRS): $$(L1) bin/matr.py
	@mkdir -p $(@D)
...
$(L1PLOTS): $$(L1CUM) bin/simple.gplot
	@mkdir -p $(@D)
