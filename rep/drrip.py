...
# Run command example:
# python3 drrip.py ../traces/ls.mtrace ../results/ls.drrip -o 2 -i 8 -w 4 -m 2 -p 0.05 -d 1
...
parser.add_option("-d", "--dtest"  , action="store", dest="dtest",     type=int, default=1, help="Number of sets (each) reserved for BRRIP and DRRIP testing")
...
# probability of using long RRPV instead of distant for BRRIP
...
# number of sets reserved (each) for testing BRRIP and SRRIP
num_test = options.dtest;


# the meter determines whether it should be using BRRIP or SRRIP
#   it will saturate at meter_size*2 and 0.
#   meter_size  -> I don't know! (default to SRRIP)
#   0           -> SRRIP
#   1           -> BRRIP
meter_size = 5;
meter = meter_size;
...
for line in input:
        ...
        if samefound == 1: 
                ...
                ## This is where the replacement algorithm matters
                        ...
                        # use SRRIP by default (meter=meter_size)
                        #   also, if we are in the reserved territory for each one
                        if ((num_test > 0 and mem_index >= 0 and mem_index < num_test) or meter <= meter_size):
                            ## SRRIP
                            # this one has now been added, make the age 'agestart' (long rrpv, not distant)
                            rrip_list[mem_index][way_select] = agestart_long
                            
                            # if we are not in the reserved ones, adjust the meter
                            if mem_index >= 2*num_test:
                                meter += 1
                        elif ((num_test > 0 and mem_index >= num_test and mem_index < 2*num_test) or meter > meter_size):
                            ## BRRIP
                        ...
                        # keep the meter in bounds
                        if meter > meter_size*2:
                            meter = meter_size*2
                        elif meter < 0:
                            meter = 0
